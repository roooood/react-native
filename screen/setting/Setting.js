...
const ARROW_ICON = <Awesome name='angle-left' size={20} color="#999" />;
...
class SettingsList extends React.Component {
  ...
  _itemView(item, index, max){
    ...
    if(item.borderHide) {
      ...
      border = index === max-1 ? {borderWidth:0} : {borderBottomWidth:0, borderColor: this.props.borderColor};
    }
    ...
    return (
      <TouchableHighlight accessible={false} key={'item_' + index} underlayColor={item.underlayColor ? item.underlayColor : this.props.underlayColor} onPress={item.onPress} onLongPress={item.onLongPress} ref={item.itemRef}>
        ...
        <View style={{justifyContent: "center",marginHorizontal:10}}>{item.icon}</View>
          {item.isAuth ?
          ...
          <View style={item.titleBoxStyle ? item.titleBoxStyle : [styles.titleBox,border, {minHeight:item.itemWidth ? item.itemWidth : this.props.defaultItemSize}]}>
            ...
            {item.hasSwitch ?
              <Switch
                ...
                : null}
            {item.hasBtn ?
              <View style={[{marginLeft:10},styles.rightSide]}>
                {item.btnIcon}
              </View>
              :null}
  ...
  itemArrowIcon(item) {
    if(item.arrowIcon) {
        return <View style={{justifyContent: "center",marginHorizontal:10}}>{item.arrowIcon}</View>;
    }

    if(item.hasNavArrow){
        return <View style={{justifyContent: "center",marginHorizontal:10}}>{ARROW_ICON}</View>;
    }
